...
当前候选版本 **0.3.11-rc2.2** 针对 DSH **0.1.5-rc.2**。升级说明与验证范围见 CHANGELOG。
...
当前候选针对 `0.1.5-rc.2` 编译并验证。RC2 原生输入按顺序发送图片和文件；文件回执只在接收会话内解析，失败会保留引用草稿。旧嵌入式 `images` 调用和旧请求摘要保持兼容。新提交在发送前保存持久化附件引用与消息身份，重试先核对原接收记录，不再次消费上传回执。其他宿主版本需要独立验证。
