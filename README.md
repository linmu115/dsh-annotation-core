...
`0.3.10-dev.2` 使用官方 DSH RC1 的公开接口，支持原生输入结算，以及可选的插件接收回执。执行器预检拒绝时保留草稿，迟到回链不会恢复已删关系，无需修改官方宿主。

宿主工具服务可用时注册 `dsh_reference_list` 和 `dsh_reference_read`，只读取当前会话已提交引用和当前执行批次。`snapshot` 保持原提交内容，`refresh` 经现有来源适配器读取，不改写原输入。分支继承引用仅提供快照，不新增回链。Codex 接入由可选的 Runtime Support 插件桥接；不授予任意笔记路径或模型删除能力。
