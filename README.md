...
`0.3.10-dev.0` 为 M5 开发版本，须搭配 M5 宿主工作树。引用提交以宿主已保存的整批接收回执结算；仅保存消息不代表已接收。执行器预检拒绝时保留草稿，迟到回链不会恢复已删关系。

宿主工具服务可用时注册 `dsh_reference_list` 和 `dsh_reference_read`，只读取当前会话已提交引用和当前执行批次。`snapshot` 保持原提交内容，`refresh` 经现有来源适配器读取，不改写原输入。分支继承引用仅提供快照，不新增回链。挂载 `executorToolBridge` 后，这两个工具可由明确选中的外部执行器使用；不授予任意笔记路径或模型删除能力。
