...
let pending: ReferenceSet = { schemaVersion: 1, setId: 'demo', profileId: 'web', sessionId: 'current', state: 'pending', revision: 1, createdAt: 1,
  items: [
    { referenceId: 'one', number: 1, sourceType: 'dsh-message', selectedText: '将引用收起为数量气泡，鼠标悬停时展开，点击后修改补充说明。', userComment: '请结合上下文解释这一段。', backlinkState: 'not-required', locator: { profileId: 'web', sessionId: 'source', anchorId: 'anchor', role: 'assistant', occurrence: 0, selectedTextHash: 'hash' } },
    { referenceId: 'two', number: 2, sourceType: 'dsh-message', selectedText: '跨会话引用保留来源位置，方便回到原文继续阅读。', userComment: '', backlinkState: 'not-required', locator: { profileId: 'web', sessionId: 'source', anchorId: 'anchor', role: 'assistant', occurrence: 0, selectedTextHash: 'hash' } },
  ] }
const listeners = new Set<() => void>()
let snapshot = { status: 'ready', revision: 1, pending }
const controller = new AnnotationDialogController()
const publish = () => { snapshot = { ...snapshot, pending }; listeners.forEach(fn => fn()); controller.replace(pending) }
