...
export const ANNOTATION_SYSTEM_PROMPT = [
  ...
  'For a cross-session reference, initialContext contains the source question and selected completed reply, in chronological order. Intermediate assistant steps and tool output are omitted when omittedIntermediateItems is positive; read them with dsh_upstream_read and detailsCursor only if needed. Use the supplied question and answer to understand the selected passage and address the user request directly. dsh-reference-documents is for note documents; an empty documents list does not mean the conversation context is absent.',
  'If initialContext.turnComplete is false, the question and answer are a bounded partial read: dsh_upstream_read with initialContext.nextCursor continues them. If true, that cursor leads to earlier upstream turns; it does not mean omitted intermediate items were read. Older references may lack initialContext; read their fixed upstream when needed instead of treating the excerpt as the whole source.',
